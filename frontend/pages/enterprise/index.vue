      ...
      <div v-else class="space-y-8 animate-fade-in">
         ...
         <div v-if="currentTab === 'Services'" class="space-y-6">
             ...
             <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                 <div>
                     <h3 class="text-xl font-bold dark:text-white">Catalogue de Services / Produits</h3>
                     <p class="text-sm text-gray-500 mt-1">Gérez vos classes, abonnements et articles en un seul endroit.</p>
                 </div>
             ...
             <div v-if="!currentEnterprise.service_groups?.length" class="text-center py-16 bg-white dark:bg-gray-800 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-700">
                 ...
                 <p class="text-gray-500 max-w-sm mx-auto mb-6">Commencez par créer un groupe de services (ex: "Scolarité", "Cantine") pour y ajouter vos prestations.</p>
                             ...
                             <div v-for="(svc, idx) in group.services" :key="svc.uid" class="relative group/svc bg-gray-50 dark:bg-gray-700/30 p-4 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-primary-200 dark:hover:border-primary-800 transition-all">
                                 ...
                                 <div class="space-y-3">
                                     ...
                                     <div v-if="svc.showConfig" class="space-y-2 mt-2 pt-2 bg-white dark:bg-gray-700 p-2 rounded-lg animate-fade-in text-xs">
                                          ...
                                          <!-- Custom Schedule UI (If applicable) -->
                                          <div v-if="svc.billing_frequency === 'CUSTOM'" class="mt-2 pt-2 border-t border-gray-100 dark:border-gray-600">
                                              <div class="flex items-center justify-between mb-2">
                                                 <span class="font-bold">Calendrier de Paiement</span>
                                                 <button @click="toggleSchedule(svc)" class="text-xs underline">{{ svc.use_schedule ? 'Mode Manuel' : 'Mode Calendrier' }}</button>
                                              </div>
                                              <div v-if="svc.use_schedule">
                                                  <div v-for="(item, sIdx) in svc.payment_schedule" :key="sIdx" class="flex items-center gap-1 mb-1">
                                                      <input v-model="item.start_date" type="date" class="w-20 text-[10px] rounded border-gray-200 p-0.5">
                                                      <span>-</span>
                                                      <input v-model="item.end_date" type="date" class="w-20 text-[10px] rounded border-gray-200 p-0.5">
                                                      <input v-model.number="item.amount" type="number" placeholder="Mnt" class="w-16 text-[10px] rounded border-gray-200 p-0.5">
                                                      <button @click="removeScheduleItem(svc, sIdx)" class="text-red-500">x</button>
                                                  </div>
                                                  <button @click="addScheduleItem(svc)" class="text-blue-600 hover:underline">+ Ajouter Période</button>
                                              </div>
                                          </div>

                                          <!-- Penalty Config (Per Request: Par Période support) -->
                                           <div class="mt-2 pt-2 border-t border-gray-100 dark:border-gray-600">
                                              <div class="flex items-center justify-between mb-1">
                                                  <span class="font-bold text-red-500">Pénalités de Retard</span>
                                                  <button @click="togglePenalty(svc)" class="text-[10px] border px-1 rounded hover:bg-gray-100">
                                                      {{ svc.penalty_config ? 'Activées' : 'Désactivées' }}
                                                  </button>
                                              </div>
                                              <div v-if="svc.penalty_config" class="space-y-1 bg-red-50 dark:bg-red-900/10 p-2 rounded">
                                                  <div class="flex gap-1">
                                                      <select v-model="svc.penalty_config.type" class="w-1/2 text-[10px] border-gray-200 rounded p-0.5">
                                                          <option value="FIXED">Montant Fixe</option>
                                                          <option value="PERCENTAGE">% du dû</option>
                                                      </select>
                                                      <input v-model.number="svc.penalty_config.value" type="number" class="w-1/2 text-[10px] border-gray-200 rounded p-0.5" placeholder="Valeur">
                                                  </div>
                                                  <div class="flex gap-1 items-center">
                                                      <span class="text-gray-500">Fréq:</span>
                                                      <select v-model="svc.penalty_config.frequency" class="flex-1 text-[10px] border-gray-200 rounded p-0.5">
                                                          <option value="ONETIME">Une fois</option>
                                                          <option value="DAILY">Chaque Jour</option>
                                                          <option value="WEEKLY">Chaque Semaine</option>
                                                          <option value="MONTHLY">Chaque Mois</option>
                                                          <!-- Per Period as requested -->
                                                          <option value="PER_PERIOD">Par Période (Echéancier)</option>
                                                      </select>
                                                  </div>
                                                  <div class="flex gap-1 items-center">
                                                      <span class="text-gray-500">Grâce:</span>
                                                      <input v-model.number="svc.penalty_config.grace_period" type="number" class="w-12 text-[10px] border-gray-200 rounded p-0.5">
         ...
         <div v-if="currentTab === 'Settings'" class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
            ...
            <!-- Service Groups Management (Redirect) -->
            <div class="mb-8 p-6 border rounded-xl dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                 <div>
                     <h4 class="font-bold text-lg dark:text-gray-200 flex items-center gap-2">
                         <FolderIcon class="w-5 h-5 text-indigo-500" />
                         Catalogue de Services
                     </h4>
                     <p class="text-sm text-gray-500 mt-1 max-w-lg">
                         Configurez vos groupes de services (ex: Scolarité, Cantine), vos tarifs, échéanciers et pénalités en détail.
                     </p>
                 </div>
                 <button @click="currentTab = 'Services'" class="whitespace-nowrap px-5 py-2.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg font-medium transition-colors border border-indigo-200 flex items-center gap-2">
