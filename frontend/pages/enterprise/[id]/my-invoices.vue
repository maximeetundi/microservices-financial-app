<template>
  <div>
    <div class="mb-8">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Mes Factures</h1>
      <p class="text-gray-500 dark:text-gray-400">Vos factures et abonnements auprès de cette entreprise</p>
    </div>

    <div v-if="loading" class="text-center py-12">
      <div class="animate-spin w-8 h-8 border-4 border-emerald-500 border-t-transparent rounded-full mx-auto"></div>
    </div>

    <div v-else class="space-y-6">
      <!-- Summary Cards -->
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm">
          <p class="text-sm text-gray-500 dark:text-gray-400">Total dû</p>
          <p class="text-2xl font-bold text-red-600 dark:text-red-400">
            {{ formatAmount(totalPending) }} XOF
          </p>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm">
          <p class="text-sm text-gray-500 dark:text-gray-400">Factures en attente</p>
          <p class="text-2xl font-bold text-amber-600 dark:text-amber-400">
            {{ pendingInvoices.length }}
          </p>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm">
          <p class="text-sm text-gray-500 dark:text-gray-400">Payées ce mois</p>
          <p class="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
            {{ paidThisMonth }}
          </p>
        </div>
      </div>

      <!-- Pending Invoices -->
      <div v-if="pendingInvoices.length > 0" class="bg-white dark:bg-slate-800 rounded-xl shadow-sm overflow-hidden">
        <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <h3 class="font-semibold text-gray-900 dark:text-white">Factures à payer</h3>
        </div>
        
        <div class="divide-y divide-gray-200 dark:divide-gray-700">
          <div v-for="invoice in pendingInvoices" :key="invoice.id" class="p-4">
            <div class="flex items-center justify-between">
              <div>
                <p class="font-medium text-gray-900 dark:text-white">{{ invoice.description || 'Facture' }}</p>
                <p class="text-sm text-gray-500 dark:text-gray-400">
                  Échéance: {{ formatDate(invoice.due_date) }}
                </p>
              </div>
              <div class="flex items-center gap-4">
                <p class="font-semibold text-gray-900 dark:text-white">
                  {{ formatAmount(invoice.amount) }} {{ invoice.currency || 'XOF' }}
                </p>
                <button 
                  @click="payInvoice(invoice)"
                  class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-medium transition-colors"
                >
                  Payer
                </button>
              </div>
            </div>
          </div>
        </div>
      </div>

      <!-- Invoice History -->
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm overflow-hidden">
        <div class="p-4 border-b border-gray-200 dark:border-gray-700">
          <h3 class="font-semibold text-gray-900 dark:text-white">Historique des factures</h3>
        </div>
        
        <div v-if="allInvoices.length === 0" class="p-8 text-center text-gray-500 dark:text-gray-400">
          <DocumentTextIcon class="w-12 h-12 mx-auto mb-3 opacity-50" />
          <p>Aucune facture trouvée</p>
        </div>

        <div v-else class="divide-y divide-gray-200 dark:divide-gray-700">
          <div v-for="invoice in allInvoices" :key="invoice.id" class="p-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-slate-700/50">
            <div class="flex items-center gap-4">
              <div class="h-10 w-10 rounded-full flex items-center justify-center"
                   :class="invoice.status === 'PAID' ? 'bg-emerald-100 dark:bg-emerald-900/30' : 'bg-amber-100 dark:bg-amber-900/30'">
                <CheckCircleIcon v-if="invoice.status === 'PAID'" class="h-5 w-5 text-emerald-600 dark:text-emerald-400" />
                <ClockIcon v-else class="h-5 w-5 text-amber-600 dark:text-amber-400" />
              </div>
              <div>
                <p class="font-medium text-gray-900 dark:text-white">{{ invoice.description || 'Facture' }}</p>
                <p class="text-sm text-gray-500 dark:text-gray-400">
                  {{ formatDate(invoice.created_at) }}
                </p>
              </div>
            </div>
            <div class="text-right">
              <p class="font-semibold text-gray-900 dark:text-white">
                {{ formatAmount(invoice.amount) }} {{ invoice.currency || 'XOF' }}
              </p>
              <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium"
                    :class="statusClass(invoice.status)">
                {{ statusLabel(invoice.status) }}
              </span>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, computed, onMounted } from 'vue'
import { useRoute } from 'vue-router'
import { enterpriseAPI } from '@/composables/useApi'
import { DocumentTextIcon, CheckCircleIcon, ClockIcon } from '@heroicons/vue/24/outline'

definePageMeta({
  layout: 'enterprise',
})

const route = useRoute()
const enterpriseId = computed(() => route.params.id as string)

const loading = ref(true)
const allInvoices = ref<any[]>([])

const pendingInvoices = computed(() => allInvoices.value.filter(i => i.status === 'PENDING' || i.status === 'OVERDUE'))
const totalPending = computed(() => pendingInvoices.value.reduce((sum, i) => sum + (i.amount || 0), 0))
const paidThisMonth = computed(() => {
  const now = new Date()
  return allInvoices.value.filter(i => {
    if (i.status !== 'PAID') return false
    const paidAt = new Date(i.paid_at)
    return paidAt.getMonth() === now.getMonth() && paidAt.getFullYear() === now.getFullYear()
  }).length
})

const fetchInvoices = async () => {
  loading.value = true
  try {
    // This would be a new endpoint to get invoices for the current user
    // For now we'll mock or use existing endpoint
    const { data } = await enterpriseAPI.getMySubscriptions(enterpriseId.value)
    // Extract invoices from subscriptions or use dedicated endpoint
    allInvoices.value = data?.invoices || []
  } catch (e) {
    console.error('Failed to load invoices', e)
  } finally {
    loading.value = false
  }
}

onMounted(fetchInvoices)

const formatDate = (date: string) => {
  if (!date) return '-'
  return new Date(date).toLocaleDateString('fr-FR', { dateStyle: 'medium' })
}

const formatAmount = (amount: number) => {
  if (!amount) return '0'
  return amount.toLocaleString('fr-FR')
}

const statusClass = (status: string) => {
  const classes: Record<string, string> = {
    'PAID': 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400',
    'PENDING': 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400',
    'OVERDUE': 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400',
  }
  return classes[status] || 'bg-gray-100 text-gray-800'
}

const statusLabel = (status: string) => {
  const labels: Record<string, string> = {
    'PAID': 'Payée',
    'PENDING': 'En attente',
    'OVERDUE': 'En retard',
  }
  return labels[status] || status
}

const payInvoice = (invoice: any) => {
  // TODO: Open payment modal
  console.log('Pay invoice', invoice)
}
</script>
