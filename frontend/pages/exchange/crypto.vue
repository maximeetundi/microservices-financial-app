  ...
  <NuxtLayout name="dashboard">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="flex items-center justify-between mb-8">
        <div>
          <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Acheter Crypto ₿</h1>
          <p class="text-gray-500 dark:text-slate-400">Achetez et vendez des cryptomonnaies</p>
        </div>
        <NuxtLink to="/exchange" class="text-indigo-500 dark:text-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-300 flex items-center gap-2 font-medium">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
          <div class="glass-card p-6 bg-white dark:bg-slate-900/80 border border-gray-100 dark:border-white/10">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-6">Échanger</h2>
            
            <!-- From Currency -->
            <div class="mb-6">
              <label class="block text-sm font-medium text-gray-500 dark:text-slate-400 mb-2">Vous payez</label>
              <div class="flex gap-4">
                <div class="flex-1">
                  <input 
                    ...
                    type="number" 
                    placeholder="0.00"
                    class="input-premium w-full text-2xl font-bold bg-gray-50 dark:bg-slate-800 text-gray-900 dark:text-white border-gray-200 dark:border-gray-700"
            ...
            <div class="flex justify-center my-4">
              <button @click="swapCurrencies" class="p-3 rounded-xl bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-500/20 dark:hover:bg-indigo-500/30 transition-colors">
            ...
            <!-- To Currency -->
            <div class="mb-6">
              <label class="block text-sm font-medium text-gray-500 dark:text-slate-400 mb-2">Vous recevez</label>
              <div class="flex gap-4">
                <div class="flex-1">
                  <input 
            ...
            <!-- Rate Info -->
            <div class="p-4 rounded-xl bg-gray-50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700/50 mb-6">
              <div class="flex justify-between text-sm mb-2">
                <span class="text-gray-500 dark:text-slate-400">Taux</span>
                <span class="text-gray-900 dark:text-white">1 {{ toCurrency }} = {{ formatMoney(rates[toCurrency] || 0, fromCurrency) }}</span>
              </div>
              <div class="flex justify-between text-sm mb-2">
                <span class="text-gray-500 dark:text-slate-400">Frais (0.5%)</span>
                <span class="text-gray-900 dark:text-white">{{ formatMoney(fee, fromCurrency) }}</span>
              </div>
              <div class="flex justify-between text-sm font-bold pt-2 border-t border-gray-200 dark:border-slate-700">
                <span class="text-gray-600 dark:text-slate-300">Total</span>
                <span class="text-gray-900 dark:text-white">{{ formatMoney(fromAmount + fee, fromCurrency) }}</span>
              </div>
            </div>
        ...
        <!-- Market Prices -->
        <div class="glass-card p-6 bg-white dark:bg-slate-900/80 border border-gray-100 dark:border-white/10">
          <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-6">Prix du marché</h2>
          <div class="space-y-4">
            <div v-for="crypto in markets" :key="crypto.symbol" 
                class="flex items-center justify-between p-3 rounded-xl bg-gray-50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700/50 hover:bg-gray-100 dark:hover:bg-slate-700/50 transition-colors cursor-pointer"
              ...
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-500 dark:text-slate-400">{{ crypto.symbol }}</p>
                </div>
              </div>
              <div class="text-right">
                <p class="font-semibold text-gray-900 dark:text-white">${{ formatPrice(crypto.price) }}</p>
                <p class="text-xs" :class="(crypto.change || 0) >= 0 ? 'text-emerald-500 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'">
...
const formatPrice = (price) => {
  ...
  return Number(price) >= 1 ? Number(price || 0).toLocaleString() : Number(price || 0).toFixed(4)
}
