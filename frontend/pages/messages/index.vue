...
const isShopConversation = (conv: any) => {
  const ctx = conv?.context
  if (!ctx) return false
  if (ctx.type === 'shop') return true
  return !!ctx.shop_id
}
...
const selectConversation = (conv: any) => {
  ...
  // Persist selected conversation in URL for deep-linking
  const query = { ...route.query, conversation_id: conv?.id }
...
onMounted(async () => {
  ...
  try {
    ...
    // Auto-open conversation if provided in URL
    const convId = typeof route.query.conversation_id === 'string' ? route.query.conversation_id : ''
    if (convId) {
      const target = userConversations.value.find((c: any) => c?.id === convId)
      if (target) {
