...
const loadMessages = async () => {
  ...
  // TODO: Load messages from messaging-service
  messages.value = []
}
...
const loadAssociationMessages = async () => {
  ...
  try {
    const res = await associationAPI.getMessages(selectedAssoc.value.id, 100)
    messages.value = (res.data?.messages || res.data || []).map((m: any) => ({
      ...m,
      isMine: false // TODO: check if current user
    }))
    await nextTick()
    scrollToBottom()
  } catch (err) {
...
onMounted(async () => {
  try {
    ...
    associationChats.value = res.data || []
  } catch (err) {
    console.error(err)
  }
})
