  ...
  <NuxtLayout name="dashboard">
    <div class="create-event-page">
      <div class="page-header">
        <NuxtLink :to="`/events/${eventId}`" class="back-link">← Annuler</NuxtLink>
        <h1 class="page-title">✏️ Modifier l'événement</h1>
      </div>
      ...
      <form v-else @submit.prevent="updateEvent" class="event-form">
        <!-- Basic Info -->
          ...
          <div class="form-group">
            ...
            <div class="image-upload-wrapper">
              ...
              <div class="upload-area" @click="$refs.coverImageInput.click()">
                <div v-if="!coverImagePreview && !form.cover_image" class="upload-placeholder">
                  <span class="upload-icon">📷</span>
                  <span>Cliquez pour modifier l'image</span>
                </div>
                <img v-else :src="coverImagePreview || form.cover_image" class="cover-preview" />
              </div>
              <button v-if="coverImagePreview || form.cover_image" type="button" @click="removeCoverImage" class="remove-image-btn">✕ Supprimer</button>
            </div>
          </div>
        </section>
        ...
        <section class="form-section">
          ...
          <p class="section-desc">Gérez vos niveaux de tickets</p>
        ...
        <div class="form-actions">
          <button type="submit" :disabled="submitting" class="btn-submit">
            <span v-if="submitting">Mise à jour en cours...</span>
...
const loadEvent = async () => {
  ...
  try {
    ...
    form.sale_start_date = formatDateForInput(event.sale_start_date)
    form.sale_end_date = formatDateForInput(event.sale_end_date)
    form.currency = event.currency
    
    // Process form fields
    form.form_fields = (event.form_fields || []).map(f => ({
      ...f,
      optionsText: f.options ? f.options.join(', ') : ''
    }))
    
    // Process tiers
    form.ticket_tiers = event.ticket_tiers || []
...
const updateEvent = async () => {
  ...
  try {
    ...
    await ticketAPI.updateEvent(eventId, payload)
    router.push(`/events/${eventId}`)
  } catch (e) {
