    ...
    if (
      ...
      try {
        const response = await axios.post(
          `${API_URL}/auth-service/api/v1/auth/refresh`,
          {
...
export const authAPI = {
  login: (email: string, password: string, twoFaCode?: string) =>
    api.post("/auth-service/api/v1/auth/login", {
      email,
      password,
  ...
  refresh: (refreshToken: string) =>
    api.post("/auth-service/api/v1/auth/refresh", {
      refresh_token: refreshToken,
    }),
  ...
  resetPassword: (token: string, password: string) =>
    api.post("/auth-service/api/v1/auth/reset-password", {
      token,
...
export const userAPI = {
  ...
  updateProfile: (data: any) =>
    api.put("/auth-service/api/v1/users/profile", data),
  ...
  verify2FA: (data: { code: string; secret?: string }) =>
    api.post("/auth-service/api/v1/users/2fa/verify", data),
  disable2FA: (data?: { code: string }) =>
    api.post("/auth-service/api/v1/users/2fa/disable", data || {}),

  // Sessions
  getSessions: () => api.get("/auth-service/api/v1/sessions"),
  revokeSession: (sessionId: string) =>
    api.delete(`/auth-service/api/v1/sessions/${sessionId}`),
  revokeAllSessions: () => api.delete("/auth-service/api/v1/sessions"),

  // PIN (5-digit transaction security PIN)
  checkPinStatus: () =>
    api.get(`/auth-service/api/v1/users/pin/status?t=${Date.now()}`),
  ...
  updatePreferences: (data: any) =>
    api.put("/auth-service/api/v1/users/preferences", data),

  // Notification Preferences
  getNotificationPrefs: () =>
    api.get("/auth-service/api/v1/users/notifications/preferences"),
  updateNotificationPrefs: (data: any) =>
    api.put("/auth-service/api/v1/users/notifications/preferences", data),
};
...
export const walletAPI = {
  ...
  getBalance: (id: string) =>
    api.get(`/wallet-service/api/v1/wallets/${id}/balance`),
  getTransactions: (id: string, limit = 50, offset = 0) =>
    api.get(
      `/wallet-service/api/v1/wallets/${id}/transactions?limit=${limit}&offset=${offset}`,
    ),
  deposit: (
    id: string,
    amount: number,
  ...
  withdraw: (id: string, amount: number, destination: string) =>
    api.post(`/wallet-service/api/v1/wallets/${id}/withdraw`, {
      amount,
      destination,
    }),
  ...
  generateCryptoWallet: (currency: string) =>
    api.post("/wallet-service/api/v1/crypto/generate", {
      currency,
      wallet_type: "crypto",
  ...
  getDepositAddress: (walletId: string, network?: string) =>
    api.get(`/wallet-service/api/v1/wallets/${walletId}/address`, {
      params: { network },
    }),
  sendCrypto: (
    walletId: string,
    data: { to_address: string; amount: number; note?: string },
  ) => api.post(`/wallet-service/api/v1/crypto/${walletId}/send`, data),
  estimateCryptoFee: (walletId: string, amount: number) =>
    api.post(`/wallet-service/api/v1/crypto/${walletId}/estimate-fee`, {
      amount,
    }),
};
...
export const transferAPI = {
  getAll: (limit = 50, offset = 0) =>
    api.get(
      `/transfer-service/api/v1/transfers?limit=${limit}&offset=${offset}`,
    ),
  ...
  getBanks: (country: string) =>
    api.get(`/transfer-service/api/v1/transfers/banks?country=${country}`),
  getMobileOperators: (country: string) =>
    api.get(
      `/transfer-service/api/v1/transfers/mobile-operators?country=${country}`,
    ),
  validateRecipient: (type: string, recipient: string) =>
    api.post("/transfer-service/api/v1/transfers/validate-recipient", {
      type,
      recipient,
    }),
  getFees: (type: string, amount: number, currency: string) =>
    api.get(
      `/transfer-service/api/v1/transfers/fees?type=${type}&amount=${amount}&currency=${currency}`,
    ),

  // Cancellation & Reversal
  cancelTransfer: (id: string, reason: string) =>
    api.post(`/transfer-service/api/v1/transfers/${id}/cancel`, { reason }),
...
export const cardAPI = {
  ...
  create: (data: { type: string; currency: string; name: string }) =>
    api.post("/card-service/api/v1/cards", data),
  activate: (id: string) =>
    api.post(`/card-service/api/v1/cards/${id}/activate`),
  freeze: (id: string) => api.post(`/card-service/api/v1/cards/${id}/freeze`),
  unfreeze: (id: string) =>
    api.post(`/card-service/api/v1/cards/${id}/unfreeze`),
  setLimit: (id: string, limitType: string, amount: number) =>
    api.post(`/card-service/api/v1/cards/${id}/limits`, {
      limit_type: limitType,
      amount,
    }),
  setPin: (id: string, pin: string) =>
    api.post(`/card-service/api/v1/cards/${id}/pin`, { pin }),
  ...
  topUp: (id: string, amount: number, sourceWalletId: string) =>
    api.post(`/card-service/api/v1/cards/${id}/topup`, {
      amount,
...
export const exchangeAPI = {
  ...
  getRate: (from: string, to: string) =>
    api.get(`/exchange-service/api/v1/rates/${from}/${to}`),
  ...
  getHistory: (limit = 50) =>
    api.get(`/exchange-service/api/v1/history?limit=${limit}`),
  getOrders: () => api.get("/exchange-service/api/v1/trading/orders"),
  getTradingPortfolio: () =>
    api.get("/exchange-service/api/v1/trading/portfolio"),
};
...
export const fiatAPI = {
  ...
  // Get fiat rates
  getRates: (base = "USD") =>
    api.get(`/exchange-service/api/v1/fiat/rates?base=${base}`),

  // Get specific fiat rate
  getRate: (from: string, to: string) =>
    api.get(`/exchange-service/api/v1/fiat/rates/${from}/${to}`),
  ...
  convert: (from: string, to: string, amount: number) =>
    api.get(
      `/exchange-service/api/v1/fiat/convert?from=${from}&to=${to}&amount=${amount}`,
    ),
};
...
export const dashboardAPI = {
  ...
  getRecentActivity: (limit = 10) =>
    api.get(`/wallet-service/api/v1/dashboard/activity?limit=${limit}`),
  getPortfolio: () => api.get("/wallet-service/api/v1/dashboard/portfolio"),
  getStats: (period: string) =>
    api.get(`/wallet-service/api/v1/dashboard/stats?period=${period}`),
};
...
export const merchantAPI = {
  ...
  getPayments: (limit = 20, offset = 0) =>
    api.get(
      `/wallet-service/api/v1/merchant/payments?limit=${limit}&offset=${offset}`,
    ),

  // Get payment history
  getHistory: (limit = 20, offset = 0) =>
    api.get(
      `/wallet-service/api/v1/merchant/payments/history?limit=${limit}&offset=${offset}`,
    ),

  // Cancel a payment request
  cancelPayment: (id: string) =>
    api.delete(`/wallet-service/api/v1/merchant/payments/${id}`),

  // Get QR code for a payment
  getQRCode: (id: string) =>
    api.get(`/wallet-service/api/v1/payments/${id}/qr`),
...
export const paymentAPI = {
  ...
  getPaymentDetails: (id: string) =>
    api.get(`/wallet-service/api/v1/pay/${id}`),
...
export const adminPaymentAPI = {
  ...
  getProvider: (id: string) =>
    api.get(`/admin-service/api/v1/admin/payment-providers/${id}`),

  // Create provider
  createProvider: (data: any) =>
    api.post("/admin-service/api/v1/admin/payment-providers", data),
  ...
  toggleStatus: (id: string, isActive: boolean) =>
    api.post(
      `/admin-service/api/v1/admin/payment-providers/${id}/toggle-status`,
      { is_active: isActive },
    ),
  ...
  toggleDemo: (id: string, isDemoMode: boolean) =>
    api.post(
      `/admin-service/api/v1/admin/payment-providers/${id}/toggle-demo`,
  ...
  addCountry: (id: string, country: any) =>
    api.post(
      `/admin-service/api/v1/admin/payment-providers/${id}/countries`,
      country,
    ),
  ...
  removeCountry: (id: string, countryCode: string) =>
    api.delete(
      `/admin-service/api/v1/admin/payment-providers/${id}/countries/${countryCode}`,
    ),
};
...
export const adminFeeAPI = {
  ...
  updateWalletFee: (data: any) =>
    api.put("/wallet-service/api/v1/admin/fees", data),
  getExchangeFees: () => api.get("/exchange-service/api/v1/admin/fees"),
  updateExchangeFee: (data: any) =>
    api.put("/exchange-service/api/v1/admin/fees", data),
};
...
export const adminPlatformAPI = {
  ...
  getAccount: (id: string) =>
    api.get(`/wallet-service/api/v1/admin/platform/accounts/${id}`),
  createAccount: (data: {
    currency: string;
    account_type: string;
    name: string;
  ...
  creditAccount: (
    id: string,
    data: { amount: number; description: string; reference?: string },
  ) =>
    api.post(
      `/wallet-service/api/v1/admin/platform/accounts/${id}/credit`,
      data,
    ),
  debitAccount: (
    id: string,
    ...
    api.post(
      `/wallet-service/api/v1/admin/platform/accounts/${id}/debit`,
      data,
    ),

  // Crypto Wallets
  getCryptoWallets: () =>
    api.get("/wallet-service/api/v1/admin/platform/crypto-wallets"),
  getCryptoWallet: (id: string) =>
    api.get(`/wallet-service/api/v1/admin/platform/crypto-wallets/${id}`),
  ...
  syncCryptoWalletBalance: (id: string) =>
    api.put(`/wallet-service/api/v1/admin/platform/crypto-wallets/${id}/sync`),

  // Transactions & Reconciliation
  getTransactions: (limit = 50, offset = 0) =>
    api.get(
      `/wallet-service/api/v1/admin/platform/transactions?limit=${limit}&offset=${offset}`,
    ),
  getReconciliation: () =>
    api.get("/wallet-service/api/v1/admin/platform/reconciliation"),
};
...
export const adminUserAPI = {
  getUsers: (limit = 20, offset = 0) =>
    api.get(
      `/admin-service/api/v1/admin/users?limit=${limit}&offset=${offset}`,
    ),
  unlockPin: (userId: string) =>
    api.post(`/auth-service/api/v1/admin/users/${userId}/unlock-pin`), // stays in auth-service
};

export const adminKYCAPI = {
  getRequests: (status = "all", limit = 20, offset = 0) =>
    api.get(
      `/admin-service/api/v1/admin/kyc/requests?status=${status}&limit=${limit}&offset=${offset}`,
    ),
  reviewRequest: (
    docId: string,
    ...
    if (status === "approved") {
      return api.post(`/admin-service/api/v1/admin/kyc/${docId}/approve`, {
        level: "full",
      }); // Default to full level
    } else {
      return api.post(`/admin-service/api/v1/admin/kyc/${docId}/reject`, {
...
export const supportAPI = {
  ...
  getTickets: (limit = 20, offset = 0) =>
    api.get(
      `/support-service/api/v1/support/conversations?limit=${limit}&offset=${offset}`,
    ),

  // Get a specific conversation
  getTicket: (id: string) =>
    api.get(`/support-service/api/v1/support/conversations/${id}`),
  ...
  sendMessage: (
    ...
    api.post(
      `/support-service/api/v1/support/conversations/${conversationId}/messages`,
      {
        content: message,
        content_type: "text",
  ...
  escalate: (conversationId: string, reason: string) =>
    api.post(
      `/support-service/api/v1/support/conversations/${conversationId}/escalate`,
      { reason },
    ),
  ...
  closeTicket: (conversationId: string, rating?: number, feedback?: string) =>
    api.put(
      `/support-service/api/v1/support/conversations/${conversationId}/close`,
      { rating, feedback },
    ),
};
...
export const notificationAPI = {
  getAll: (options: { limit?: number; offset?: number } = {}) => {
    ...
    return api.get(
      `/notification-service/api/v1/notifications?limit=${limit}&offset=${offset}`,
    );
  },
  getUnreadCount: () =>
    api.get("/notification-service/api/v1/notifications/unread-count"),
  markAsRead: (id: string) =>
    api.post(`/notification-service/api/v1/notifications/${id}/read`),
  markAllAsRead: () =>
    api.post("/notification-service/api/v1/notifications/read-all"),
  delete: (id: string) =>
    api.delete(`/notification-service/api/v1/notifications/${id}`),
};
...
export const ticketAPI = {
  ...
  getMyEvents: (limit = 20, offset = 0) =>
    api.get(`/ticket-service/api/v1/events?limit=${limit}&offset=${offset}`),
  getEvent: (id: string) => api.get(`/ticket-service/api/v1/events/${id}`),
  getEventByCode: (code: string) =>
    api.get(`/ticket-service/api/v1/events/code/${code}`),
  updateEvent: (id: string, data: any) =>
    api.put(`/ticket-service/api/v1/events/${id}`, data),
  deleteEvent: (id: string) =>
    api.delete(`/ticket-service/api/v1/events/${id}`),
  publishEvent: (id: string) =>
    api.post(`/ticket-service/api/v1/events/${id}/publish`),
  getEventTickets: (id: string, limit = 50, offset = 0) =>
    api.get(
      `/ticket-service/api/v1/events/${id}/tickets?limit=${limit}&offset=${offset}`,
    ),
  getEventStats: (id: string) =>
    api.get(`/ticket-service/api/v1/events/${id}/stats`),

  // Public events
  getActiveEvents: (limit = 20, offset = 0) =>
    api.get(
      `/ticket-service/api/v1/events/active?limit=${limit}&offset=${offset}`,
    ),
  getPublicEvent: (id: string) =>
    api.get(`/ticket-service/api/v1/events/public/${id}`),
  ...
  getMyTickets: (limit = 20, offset = 0) =>
    api.get(`/ticket-service/api/v1/tickets?limit=${limit}&offset=${offset}`),
  getTicket: (id: string) => api.get(`/ticket-service/api/v1/tickets/${id}`),

  // Ticket verification
  verifyTicket: (code: string) =>
    api.post("/ticket-service/api/v1/tickets/verify", { ticket_code: code }),
  useTicket: (id: string) =>
    api.post(`/ticket-service/api/v1/tickets/${id}/use`),
  ...
  refundTicket: (id: string, reason: string, pin?: string) =>
    api.post(`/ticket-service/api/v1/tickets/${id}/refund`, { reason, pin }),
  cancelEvent: (id: string, reason: string, pin?: string) =>
    api.post(`/ticket-service/api/v1/events/${id}/cancel`, { reason, pin }),
};
...
export const donationAPI = {
  ...
  createCampaign: (data: any) =>
    api.post("/donation-service/api/v1/campaigns", data),
  getCampaigns: (limit = 20, offset = 0) =>
    api.get(
      `/donation-service/api/v1/campaigns?limit=${limit}&offset=${offset}`,
    ),
  getMyCampaigns: (creatorID: string) =>
    api.get(`/donation-service/api/v1/campaigns?creator_id=${creatorID}`),
  getCampaign: (id: string) =>
    api.get(`/donation-service/api/v1/campaigns/${id}`),
  updateCampaign: (id: string, data: any) =>
    api.put(`/donation-service/api/v1/campaigns/${id}`, data),
  cancelCampaign: (id: string, reason: string, pin: string) =>
    api.post(`/donation-service/api/v1/campaigns/${id}/cancel`, {
      reason,
      pin,
    }),

  // Donations
  initiateDonation: (data: any) =>
    api.post("/donation-service/api/v1/donations", data),
  refundDonation: (id: string, reason: string, pin?: string) =>
    api.post(`/donation-service/api/v1/donations/${id}/refund`, {
      reason,
      pin,
    }),
  getDonations: (campaignID: string, limit = 20, offset = 0) =>
    api.get(
      `/donation-service/api/v1/donations?campaign_id=${campaignID}&limit=${limit}&offset=${offset}`,
    ),
...
export const contactsAPI = {
  ...
  // Delete a contact
  delete: (id: string) =>
    api.delete(`/auth-service/api/v1/users/contacts/${id}`),

  // Lookup contact name by phone or email
  lookup: (phone?: string, email?: string) =>
    api.get("/auth-service/api/v1/users/contacts/lookup", {
      params: { phone, email },
    }),
};
...
export const enterpriseAPI = {
  ...
  getNotifications: (entId: string, limit = 20, offset = 0) =>
    api.get(
      `/enterprise-service/api/v1/enterprises/${entId}/notifications?limit=${limit}&offset=${offset}`,
    ),

  // Employee Management
  listEmployees: (entId: string) =>
    api.get(`/enterprise-service/api/v1/enterprises/${entId}/employees`),
  inviteEmployee: (entId: string, data: any) =>
    api.post("/enterprise-service/api/v1/employees/invite", {
      ...data,
      enterprise_id: entId,
    }),
  acceptInvitation: (data: { employee_id: string; pin?: string }) =>
    api.post("/enterprise-service/api/v1/employees/accept", data),
  getInvitationDetails: (employeeId: string) =>
    api.get(`/enterprise-service/api/v1/invitations/${employeeId}`),
  promoteEmployee: (id: string, data: any) =>
    api.put(`/enterprise-service/api/v1/employees/${id}/promote`, data),
  getMyEmployee: (entId: string) =>
    api.get(`/enterprise-service/api/v1/employees/me`, {
      params: { enterprise_id: entId },
    }),
  getMySubscriptions: (entId: string) =>
    api.get(`/enterprise-service/api/v1/enterprises/${entId}/subscriptions/me`),
  getMySalary: (entId: string) =>
    api.get(`/enterprise-service/api/v1/employees/me/salary`, {
      params: { enterprise_id: entId },
    }),

  // Payroll
  previewPayroll: (entId: string) =>
    api.post(`/enterprise-service/api/v1/enterprises/${entId}/payroll/preview`),
  executePayroll: (entId: string) =>
    api.post(`/enterprise-service/api/v1/enterprises/${entId}/payroll/execute`),

  // Billing & Invoicing
  createInvoice: (data: any) =>
    api.post("/enterprise-service/api/v1/invoices", data),
  importInvoices: (entId: string, formData: FormData) =>
    api.post(
  ...
  scheduleBatch: (entId: string, batchId: string, scheduledAt: string) =>
    api.post(
      `/enterprise-service/api/v1/enterprises/${entId}/invoices/batches/${batchId}/schedule`,
      { scheduled_at: scheduledAt },
    ),

  // Subscriptions & Manual Entry
  getSubscriptions: (entId: string, serviceId?: string) =>
    api.get(`/enterprise-service/api/v1/enterprises/${entId}/subscriptions`, {
      params: { service_id: serviceId },
    }),
  createSubscription: (entId: string, data: any) =>
    api.post(
      `/enterprise-service/api/v1/enterprises/${entId}/subscriptions`,
      data,
    ),
  createBatchInvoices: (
    entId: string,
    ...
    api.post(
      `/enterprise-service/api/v1/enterprises/${entId}/invoices/batches`,
      { items },
    ),

  // Multi-Approval System
  getPendingApprovals: (entId: string) =>
    api.get(`/enterprise-service/api/v1/enterprises/${entId}/approvals`),
  getApprovalById: (approvalId: string) =>
    api.get(`/enterprise-service/api/v1/approvals/${approvalId}`),
  approveAction: (approvalId: string, data: { pin: string }) =>
    api.post(
      `/enterprise-service/api/v1/approvals/${approvalId}/approve`,
      data,
    ),
  ...
  exportData: (entId: string) =>
    api.get(`/enterprise-service/api/v1/enterprises/${entId}/export`, {
      responseType: "blob",
    }),
  getExportStatus: (entId: string) =>
    api.get(`/enterprise-service/api/v1/enterprises/${entId}/export/status`),
};
...
export const adminGlobalFeeAPI = {
  ...
  update: (key: string, data: any) =>
    api.put(`/admin-service/api/v1/admin/fees/${key}`, data),
};
...
export const messagingAPI = {
  ...
  getMessages: (conversationId: string) =>
    api.get(
      `/messaging-service/api/v1/conversations/${conversationId}/messages`,
    ),
  sendMessage: (
    conversationId: string,
    content: string,
  ...
  markAsRead: (messageId: string) =>
    api.post(`/messaging-service/api/v1/messages/${messageId}/read`),

  // Upload
  uploadFile: (formData: FormData) =>
    api.post("/messaging-service/api/v1/upload", formData, {
      headers: { "Content-Type": "multipart/form-data" },
    }),
};
...
export const systemConfigAPI = {
  ...
  getNetworks: (currency: string) =>
    api.get(`/wallet-service/api/v1/public/networks?currency=${currency}`),
};
