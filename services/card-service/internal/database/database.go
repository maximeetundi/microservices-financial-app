...
func InitializeRabbitMQ(url string) (*RabbitMQClient, error) {
	...
	// Declare queues for card service
	queues := []string{"card_transactions", "card_notifications", "card.wallet_updates"}
	...
	// Bind card.wallet_updates to wallet.events for balance confirmations
	err = ch.QueueBind(
		"card.wallet_updates",
		"wallet.*",
		"wallet.events",
