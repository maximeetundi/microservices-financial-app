...
// CalculateFee calculates the fee based on the config key and amount
func (s *FeeService) CalculateFee(key string, amount float64) (float64, error) {
	...
	// Round to 8 decimal places to avoid float precision issues
	return math.Round(fee*100000000) / 100000000, nil
}

func (s *FeeService) GetAllConfigs() ([]models.FeeConfig, error) {
	return s.repo.GetAll()
}

func (s *FeeService) UpdateConfig(config *models.FeeConfig) error {
