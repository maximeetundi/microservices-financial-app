...
func (s *StripeCollectionProvider) InitiateCollection(ctx context.Context, req *CollectionRequest) (*CollectionResponse, error) {
	...
	payload := fmt.Sprintf(
		"mode=payment&success_url=%s&cancel_url=%s&line_items[0][price_data][currency]=%s&line_items[0][price_data][product_data][name]=Wallet+Deposit&line_items[0][price_data][unit_amount]=%d&line_items[0][quantity]=1&metadata[reference_id]=%s&metadata[user_id]=%s",
		req.RedirectURL,
		req.RedirectURL,
		req.Currency,
...
func (s *StripeCollectionProvider) VerifyCollection(ctx context.Context, sessionID string) (*CollectionResponse, error) {
	...
	return &CollectionResponse{
		ReferenceID:       result.Metadata.ReferenceID,
		ProviderReference: result.ID,
		Status:            status,
